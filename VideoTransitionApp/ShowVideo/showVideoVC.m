...
//    [self logAllFilters];
    ...
    canvasHeight = 375;
    canvasWidth = 375;
    self.Height.constant = canvasHeight;
    self.Width.constant = canvasWidth;
...
//    CIContext *context = [CIContext contextWithOptions:nil];
    return [AVMutableVideoComposition videoCompositionWithAsset:composition applyingCIFiltersWithHandler:^(AVAsynchronousCIImageFilteringRequest * _Nonnull request) {
        CIImage *sourceImage = request.sourceImage;
        AVAssetTrack *sourceVideoTrack = [[self->resultAsset1 tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0];
        CGSize temp = CGSizeApplyAffineTransform(sourceVideoTrack.naturalSize, sourceVideoTrack.preferredTransform);
        CGSize size = CGSizeMake(fabs(temp.width), fabs(temp.height));
        CGAffineTransform transform = self->resultAsset1.preferredTransform;
        NSLog(@"%f",self->canvasWidth);
        float s = self->canvasWidth/size.width;
        CGAffineTransform new = CGAffineTransformConcat(transform, CGAffineTransformMakeScale(s,s));
        CGAffineTransform newer;
        if(self->canvasWidth==200){
         newer = CGAffineTransformConcat(new, CGAffineTransformMakeTranslation(((375-200)/2.0)+1, 0));
        }
        
        else if(self->canvasWidth == 250)
        {
            newer = CGAffineTransformConcat(new, CGAffineTransformMakeTranslation(((375-250)/2)+1, 0));
        }
        
        else newer = CGAffineTransformConcat(new, CGAffineTransformMakeTranslation(0, 0));
        CIImage *img = request.sourceImage;
        CGAffineTransform backNew = CGAffineTransformConcat(transform, CGAffineTransformMakeScale(self->canvasWidth/img.extent.size.width,self->canvasHeight/img.extent.size.height));
        CGAffineTransform backnewer ;
        
        if(self->canvasWidth==200){
            backnewer = CGAffineTransformConcat(backNew, CGAffineTransformMakeTranslation(((375-200)/2.0)+1, 0));
        }
        else if(self->canvasWidth == 250)
        {
            backnewer = CGAffineTransformConcat(backNew, CGAffineTransformMakeTranslation(((375-250)/2)+1, 0));
        }
        else backnewer = CGAffineTransformConcat(backNew, CGAffineTransformMakeTranslation(0,0));
        
        
        
        img = [img imageByApplyingTransform:backnewer];
        [filter setValue:img forKey:kCIInputImageKey];
        [filter setValue:@(5.0) forKey:kCIInputRadiusKey];
        sourceImage = [ sourceImage imageByApplyingTransform:newer];
        [blend setValue:filter.outputImage forKey:kCIInputBackgroundImageKey];
...
- (IBAction)sixtofiveRation:(id)sender {
    self.Height.constant = 375 * (5.f/4.f);
    self.Width.constant = 375;
    canvasWidth = 375;
    canvasHeight =375 * (5.f/4.f);
    [self.view layoutIfNeeded];
    
}

- (IBAction)anotherRation:(id)sender {
    self.Height.constant = 375;
